    ...
    if (library) {
      ...
      try {
        ...
        if (state.numOfPasses == 1) {
          ...
          data = await contract
            .connect(library.getSigner())
            .batchSafeMint(state.numOfPasses, account, {
  ...
  return (
    <Layout>
      ...
      <Text mt="8" fontSize="xl">
        Connect your wallet first! This page only works on the RINKEBY Testnet
        or on a Local Chain.
      </Text>
      <Box maxWidth="container.xm" p="8" mt="8" bg="gray.100">
        ...
        <Box>
          <Text fontSize="lg">
            Minted supply: {state.tokenSupply} | Maximum Supply:{' '}
            {state.maxSupply}{' '}
          </Text>
        ...
        <Box>
          <Link
            mt="2"
            href={`https://rinkeby.etherscan.io/tx/${state.txHashValue}`}
            isExternal
          >
